              ...
              <div className="acc__template-section">
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', margin: '0 0 12px', padding: '10px 12px', background: 'rgba(128,128,128,0.08)', borderRadius: 10 }}>
                  <div>
                    ...
                    <p style={{ margin: '3px 0 0', fontSize: 11, color: 'var(--t-text-muted)', opacity: 0.8 }}>
                      {debugMode ? 'Locked — backend won\'t override' : 'Backend controls on tab switch'}
                    </p>
                  </div>
                  <div
                    onClick={() => { const next = !debugMode; setDebugTemplateMode(next); setDebugMode(next); }}
                    style={{ width: 44, height: 24, borderRadius: 12, background: debugMode ? `var(--t-primary, ${template.colors.primary})` : '#999', position: 'relative', cursor: 'pointer', transition: 'background 0.2s', flexShrink: 0, marginLeft: 12 }}
                  >
